...
#选择运行模式 test: 可选爬取页数，耗时短 normal：正常运行，耗时长           
    ...
    if mode == 1:
        maxpage=int(input("Please input max pages:"))
        modetype = maxpage
    ...
    cat_response = requests.get(url=source_page_url, headers=buff_headers, cookies=buff_cookies)
    html_text0 = cat_response.text
    category_list = re.findall( r'li value="weapon_(.+?)">', html_text0, re.M)
    
    if mode == 2:
        modetype = len(category_list)
    #爬取单个武器页数 
    for i in range(modetype):
        ...
        page_response = requests.get(url=first_url, headers=buff_headers, cookies=buff_cookies)
        ...
        for j in range(page_num):
            ...
            page_response = requests.get(url=page_url, headers=buff_headers, cookies=buff_cookies)
            html_text2 = page_response.text
            #爬取饰品中文名
            names_list_temp = re.findall(r'"name": "(.*)",', html_text2, re.M)
            #将饰品名转化为中文
            ...
            #爬取steam最低售价
            steam_price_list_temp = re.findall(r'"steam_price_cny": "(.*)"',html_text2,re.M)
            names_list += names_list_temp
            price_list += price_list_temp
            steam_price_list += steam_price_list_temp
            time.sleep(15)#停止爬取15秒，防止封号
    ...
    items_information = pandas.DataFrame(columns=excel_name, data=excel_data)
